...
$sqldate = "SELECT MIN(s_requestDate) FROM tb_service";
...
if($filter){
    $sql = "SELECT * FROM tb_service 
        ...
        WHERE s_statusID = '$filter' AND YEAR(s_requestDate) = '$fyear' AND MONTH(s_requestDate) = '$fmonth'
        ORDER BY s_requestDate DESC" ;
    ...
    $sql = "SELECT * FROM tb_service 
        ...
        LEFT JOIN tb_status ON tb_service.s_statusID = tb_status.st_statusID
        LEFT JOIN tb_type ON tb_service.s_typeID = tb_type.t_typeID
        WHERE YEAR(s_requestDate) = '$fyear' AND MONTH(s_requestDate) = '$fmonth'
        ORDER BY s_requestDate DESC" ;
}
...
$sql1 = "SELECT COUNT(*) FROM tb_service WHERE s_statusID = '3' AND YEAR(s_requestDate) = '$fyear' AND MONTH(s_requestDate) = '$fmonth'";
$result1 = mysqli_query($con, $sql1);
$row1 = mysqli_fetch_array($result1);

$sql2 = "SELECT COUNT(*) FROM tb_service WHERE s_statusID = '2' AND YEAR(s_requestDate) = '$fyear' AND MONTH(s_requestDate) = '$fmonth'";
$result2 = mysqli_query($con, $sql2);
$row2 = mysqli_fetch_array($result2);

$sql3 = "SELECT COUNT(*) FROM tb_service WHERE s_statusID = '1' AND YEAR(s_requestDate) = '$fyear' AND MONTH(s_requestDate) = '$fmonth'";
$result3 = mysqli_query($con, $sql3);
$row3 = mysqli_fetch_array($result3);

$sql4 = "SELECT COUNT(*) FROM tb_service WHERE s_statusID = '5' AND YEAR(s_requestDate) = '$fyear' AND MONTH(s_requestDate) = '$fmonth'";
$result4 = mysqli_query($con, $sql4);
$row4 = mysqli_fetch_array($result4);

$sql5 = "SELECT COUNT(*) FROM tb_service WHERE s_statusID = '4' AND YEAR(s_requestDate) = '$fyear' AND MONTH(s_requestDate) = '$fmonth'";
$result5 = mysqli_query($con, $sql5);
$row5 = mysqli_fetch_array($result5);

$total = $row4[0] + $row3[0] + $row2[0] + $row1[0] + $row5[0]  ;
if($total==0){
    $total = 1;
}
...
$p5 = number_format((float)($row5[0]/$total*100), 2, '.', '');
...
<div class="page-breadcrumb bg-white">
    <div class="row align-items-center">
        <div class = "col-lg-3 col-md-4 col-sm-4 col-xs-4">
            <h4 class="page-title"> 
                <a href="servicereport.php" style="color:black;">Service Report</a>
            </h4>
        </div>
        <div class="col-lg-3 col-md-3 col-sm-3 col-xs-3" >
                ...
                <?php 
                        echo '<select class="form-select" onchange="location = this.value;" style="width:130px; position: absolute; right:150px; top :5px;" >';
                        echo '<option value="" hidden>-Select Month-</option>';
                        echo '<option value="servicereportmonth.php?filter='.$filter.'&year='.$fyear.'&month=1">Jan</option>';
                        echo '<option value="servicereportmonth.php?filter='.$filter.'&year='.$fyear.'&month=2">Feb</option>';
                        echo '<option value="servicereportmonth.php?filter='.$filter.'&year='.$fyear.'&month=3">Mar</option>';
                        echo '<option value="servicereportmonth.php?filter='.$filter.'&year='.$fyear.'&month=4">Apr</option>';
                        echo '<option value="servicereportmonth.php?filter='.$filter.'&year='.$fyear.'&month=5">May</option>';
                        echo '<option value="servicereportmonth.php?filter='.$filter.'&year='.$fyear.'&month=6">Jun</option>';
                        echo '<option value="servicereportmonth.php?filter='.$filter.'&year='.$fyear.'&month=7">Jul</option>';
                        echo '<option value="servicereportmonth.php?filter='.$filter.'&year='.$fyear.'&month=8">Aug</option>';
                        echo '<option value="servicereportmonth.php?filter='.$filter.'&year='.$fyear.'&month=9">Sep</option>';
                        echo '<option value="servicereportmonth.php?filter='.$filter.'&year='.$fyear.'&month=10">Oct</option>';
                        echo '<option value="servicereportmonth.php?filter='.$filter.'&year='.$fyear.'&month=11">Nov</option>';
                        echo '<option value="servicereportmonth.php?filter='.$filter.'&year='.$fyear.'&month=12">Dec</option>';
                        echo '</select>';

                        echo '<select class="form-select" onchange="location = this.value;" style="width:130px; position: absolute; right:10px; top :5px;" >';
                        while ($cyear != ($pyear-1)){
                            if ($cyear == $fyear){
                                echo '<option value="servicereportyear.php?filter='.$filter.'&year='.$cyear.'" selected>'.$cyear.'</option>';
                            }
                            else{
                                echo '<option value="servicereportyear.php?filter='.$filter.'&year='.$cyear.'">'.$cyear.'</option>';
                            }
                            $cyear--;
                        }
	...
	<div class="row">
        <div class="col">
          ...
          <div class="row">
            ...
            <a href="servicereportmonth.php?filter=3&year=<?php echo $fyear;?>&month=<?php echo $fmonth;?>">
            ...
            <div class="col">
            <a href="servicereportmonth.php?filter=2&year=<?php echo $fyear;?>&month=<?php echo $fmonth;?>">
            ...
            <div class="col">
            <a href="servicereportmonth.php?filter=1&year=<?php echo $fyear;?>&month=<?php echo $fmonth;?>">
            ...
            <div class="col">
            <a href="servicereportmonth.php?filter=5&year=<?php echo $fyear;?>&month=<?php echo $fmonth;?>">
            ...
            <div class="col">
            <a href="servicereportyear.php?filter=4&year=<?php echo $fyear;?>">
              <div class="dash5 white-box analytics-info">
                <h3 class="box-title">Completed </h3>
                <ul class="list-inline two-part d-flex align-items-center mb-0">
                  <li class="ms-auto">
                    <span class="counter text"><?php echo $row5[0];?></span>
                  </li>
                </ul>
              </div>
              </a>
...
<div class="row">
    ...
    <?php 
        ...
        if ($filter == 1){
            ...
            $f = ' Completed ';
            echo' <a class="text-primary"> Completed </a>';
        }
        else if ($filter == 5){
            $f = ' Deleted ';
            echo' <a class="text-secondary"> Deleted </a>'; 
        }
        echo 'Service : '.mysqli_num_rows($result).'</h3>'; 
        echo '</div>';


    echo '<div class="col-5 ms-auto" style="display:flex; justify-content: right;">
    <a id="printbtn" type="button" class="btn btn-primary btn-lg" onClick="printpage('.$fyear.', '.$fmonth.', '.$filter.');" style="width:200px; margin-bottom:10px;">Print</a>
</div>';
                    ...
                    <table class="table table-hover">
                        <thead>
                            <tr>
                                ...
                                <th class="border-top-0">Completed Date</th>
                        ...
                        <tbody>
                            <?php
                                ...
                                while ($row = mysqli_fetch_array($result)){
                                    ...
                                    echo "<td> ".$row['s_requestDate']."</td>";
                                    echo "<td> ".$row['s_completeDate']."</td>";
                                        if($row['s_statusID']=='1'){
                                    ...
                                    echo "<tr>";
                                    echo "<td>No service in this month...</td>";
                                    echo "</tr>";
                                }
                            ?>    
...
<?php
    ...
    <a id="printbtn" type="button" class="btn btn-primary btn-lg" onClick="printpage('.$fyear.', '.$fmonth.', '.$filter.');" style="width:200px; margin-left:20px;margin-bottom:10px;">Print</a>
';
...
<script>
    ...
    function printpage(fy,m, f) {
        ...
        mywindow.document.write('<h3 style="display:block;">POWEREC '+ filter + ' Service Report Year ' +month+' '+ fy +'</h3>');
