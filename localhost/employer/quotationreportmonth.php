...
if($filter){
    $sql = "SELECT * FROM tb_quotation 
        ...
        WHERE q_statusID = '$filter' AND YEAR(q_date) = '$fyear' AND MONTH(q_date) = '$fmonth'
    ...
    $sql = "SELECT * FROM tb_quotation 
        ...
        LEFT JOIN tb_status ON tb_quotation.q_statusID = tb_status.st_statusID
        LEFT JOIN tb_customer ON tb_service.s_custID = tb_customer.c_custID
        WHERE YEAR(q_date) = '$fyear' AND MONTH(q_date) = '$fmonth'
        ORDER BY q_date DESC" ;
}
...
$sql1 = "SELECT COUNT(*) FROM tb_quotation WHERE q_statusID = '3' AND YEAR(q_date) = '$fyear' AND MONTH(q_date) = '$fmonth'";
$result1 = mysqli_query($con, $sql1);
$row1 = mysqli_fetch_array($result1);

$sql2 = "SELECT COUNT(*) FROM tb_quotation WHERE q_statusID = '2' AND YEAR(q_date) = '$fyear' AND MONTH(q_date) = '$fmonth'";
$result2 = mysqli_query($con, $sql2);
$row2 = mysqli_fetch_array($result2);

$sql3 = "SELECT COUNT(*) FROM tb_quotation WHERE q_statusID = '1' AND YEAR(q_date) = '$fyear' AND MONTH(q_date) = '$fmonth'";
$result3 = mysqli_query($con, $sql3);
$row3 = mysqli_fetch_array($result3);

$sql4 = "SELECT COUNT(*) FROM tb_quotation WHERE q_statusID = '5' AND YEAR(q_date) = '$fyear' AND MONTH(q_date) = '$fmonth'";
    ...
    <div class="row align-items-center">
        ...
        <div class="col-lg-3 col-md-3 col-sm-3 col-xs-3" >
                ...
                <?php 
                        ...
                        echo '<option value="quotationreportmonth.php?filter='.$filter.'&year='.$fyear.'&month=1">Jan</option>';
                        echo '<option value="quotationreportmonth.php?filter='.$filter.'&year='.$fyear.'&month=2">Feb</option>';
                        echo '<option value="quotationreportmonth.php?filter='.$filter.'&year='.$fyear.'&month=3">Mar</option>';
                        echo '<option value="quotationreportmonth.php?filter='.$filter.'&year='.$fyear.'&month=4">Apr</option>';
                        echo '<option value="quotationreportmonth.php?filter='.$filter.'&year='.$fyear.'&month=5">May</option>';
                        echo '<option value="quotationreportmonth.php?filter='.$filter.'&year='.$fyear.'&month=6">Jun</option>';
                        echo '<option value="quotationreportmonth.php?filter='.$filter.'&year='.$fyear.'&month=7">Jul</option>';
                        echo '<option value="quotationreportmonth.php?filter='.$filter.'&year='.$fyear.'&month=8">Aug</option>';
                        echo '<option value="quotationreportmonth.php?filter='.$filter.'&year='.$fyear.'&month=9">Sep</option>';
                        echo '<option value="quotationreportmonth.php?filter='.$filter.'&year='.$fyear.'&month=10">Oct</option>';
                        echo '<option value="quotationreportmonth.php?filter='.$filter.'&year='.$fyear.'&month=11">Nov</option>';
                        echo '<option value="quotationreportmonth.php?filter='.$filter.'&year='.$fyear.'&month=12">Dec</option>';
                        echo '</select>';

                        echo '<select class="form-select" onchange="location = this.value;" style="width:130px; position: absolute; right:10px; top :5px;" >';
                        while ($cyear != ($pyear-1)){
                            if ($cyear == $fyear){
                                echo '<option value="quotationreportyear.php?filter='.$filter.'&year='.$cyear.'" selected>'.$cyear.'</option>';
                            }
                            else{
                                echo '<option value="quotationreportyear.php?filter='.$filter.'&year='.$cyear.'">'.$cyear.'</option>';
                            }
                            $cyear--;
                        }
	...
	<div class="row">
        <div class="col-md-12">
          ...
          <div class="row">
            ...
            <a href="quotationreportmonth.php?filter=3&year=<?php echo $fyear;?>&month=<?php echo $fmonth;?>">
            ...
            <div class="col-lg-3 col-md-12">
            <a href="quotationreportmonth.php?filter=2&year=<?php echo $fyear;?>&month=<?php echo $fmonth;?>">
            ...
            <div class="col-lg-3 col-md-12">
            <a href="quotationreportmonth.php?filter=1&year=<?php echo $fyear;?>&month=<?php echo $fmonth;?>">
            ...
            <div class="col-lg-3 col-md-12">
            <a href="quotationreportmonth.php?filter=5&year=<?php echo $fyear;?>&month=<?php echo $fmonth;?>">
                        ...
                        <tbody>
                            <?php
                                ...
                                while ($row = mysqli_fetch_array($result)){
                                    ...
                                    echo "<tr>";
                                    echo "<td>No Quotation in this month...</td>";
                                    echo "</tr>";
                                }
                            ?>    
...
<script>
 function exportReportToExcel() {
    ...
    function printpage(fy,m, f) {
        ...
        mywindow.document.write('<h3 style="display:block;">POWEREC '+ filter + ' Quotation Report Year ' +month+' '+ fy +'</h3>');
